...
Pada tingkat dasar, kita dapat mengkategorikan pendekatan AMR parsing menjadi dua kelas, yaitu dua tahap \textit{parsing} dan satu tahap \textit{parsing} \citek{cai2020}.
...
\fig{4-1-partially-constructing-amr-graph}
  ...
  {
    Pembangunan sebuah graf \gls{AMR} dari subgraf \gls{AMR} yang sebagian terbentuk \citek{cai2020}.
...
\textcite{cai2020} mengusulkan pendekatan \amrparsing{} via \textit{graph-sequence iterative inference} yang meniru proses manusia dalam melakukan deduksi graf semantik dari suatu kalimat.
...
\fig[1]{4-1-dual-graph-sequence-iterative-inference}
  ...
  {Ilustrasi dari pendekatan \textit{graph-sequence iterative inference} untuk \amrparsing{} \citek{cai2020}.}
